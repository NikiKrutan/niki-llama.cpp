...
struct common_speculative_impl {
    ...
    // force the implementation to bring its KV cache in sync with the target up to the
    // current position. used by MTP to drain a backlog accumulated while process() was
    // being skipped (a higher-priority impl was drafting). the server calls this before
    // serializing the draft context into the slot / prompt cache so a consistent ctx_dft
    // is written. no-op by default.
    virtual void flush(llama_seq_id /*seq_id*/) {}

    // reset per-seq speculative state for a new prompt (e.g. MTP's skip/catch-up backlog).
    // called by the server at prompt start, BEFORE the prompt is prefilled, so a stale
    // skip_process from the previous task does not make the prefill stash instead of sync.
    // no-op by default.
    ...
    // if false, process() will not be called for this implementation on a given batch.
    // MTP used to return false here while skipped to avoid the redundant ctx_dft decode,
    // but it now always returns true so it can stash the target's nextn embeddings into its
    // catch-up backlog even while skipped (the decode itself is skipped inside process()).
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    // Always true: even while skipped we must run process() so it can stash the target's
    // nextn embeddings into the catch-up backlog. the expensive ctx_dft decode is skipped
    // inside process() itself (see the skip_process branch); this hook only gates whether
    // process() is invoked at all.
    bool need_process() const override {
        return true;
    }
};
